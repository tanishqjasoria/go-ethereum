...
func TestAccessWitness(t *testing.T) {

	aw := types.NewAccessWitness()
	var gas uint64
	gas = 0

	gas = gas + aw.TouchTxOriginAndComputeGas(common.Address{1}.Bytes())
	gas = gas + aw.TouchTxExistingAndComputeGas(common.Address{2}.Bytes(), false)
	fmt.Println(gas)
}
